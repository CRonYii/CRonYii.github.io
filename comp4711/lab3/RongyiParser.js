...
const { isValidChar, isValidStartChar, isWhiteSpace } = (function () {
    ...
    return { isValidChar, isValidStartChar, isWhiteSpace };
})();
...
class RongyiParser {
    ...
    parseFromString(input) {
        ...
        return RongyiParser.toDOMElement(this.result);
    }
...
RongyiParser.toDOMElement = (result) => {
    const ele = document.createElement(result.name);
    result.attributes.forEach(({ key, value }) => {
    ...
    result.children.forEach((child) => {
        if (typeof child === 'string') {
            ...
            ele.append(RongyiParser.toDOMElement(child));
        }
    });
    return ele;
}